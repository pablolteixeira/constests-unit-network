...
pub use pallet_oracle::DefaultCombineData;
...
pub use asset_balances;
pub use pallet_ranks;
pub use stores;
pub use collectibles;
pub use pallet_profile;
pub use pallet_questions;
pub use pallet_teams_advisors;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("node-unitchain"),
	impl_name: create_runtime_str!("node-unitchain"),
	authoring_version: 1,
...
parameter_types! {
    pub const AssetDeposit: Balance = 0 * DOLLARS;
    pub const ApprovalDeposit: Balance = 0 * DOLLARS;
    pub const StringLimit: u32 = 50;
    pub const MetadataDepositBase: Balance = 0 * DOLLARS;
    pub const MetadataDepositPerByte: Balance = 0 * DOLLARS;
}
...
impl blogchain::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type Currency = Balances; 
...
impl stores::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
}
...
impl pallet_teams_advisors::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
}

impl pallet_questions::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type Fungibles = Assets;
	type MaxLength = ConstU32<50>;
	// type MinAmountToAsk = ConstU128<100_000_000_000>;
	type MinAmountToAsk = ConstU128<0>;
}


impl pallet_ranks::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
}

impl asset_balances::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;	
}
...
parameter_types! {
    pub const DexPalletId: PalletId = PalletId(*b"dex_abcd");
}

parameter_types! {
	pub const MaxOracleFeeder: u32 = 10;
}
...
impl pallet_membership::Config for Runtime {
	...
	type MaxMembers = MaxOracleFeeder;
	type WeightInfo = ();
}
...
impl pallet_dex::Config for Runtime {
    ...
    type WeightInfo = ();
    // Provider fee is 0.3%
    type ProviderFeeNumerator = ConstU128<2>;
...
impl pallet_papers::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type AssetBalance = u128;
	type Assets = Assets;
	type Currency = Balances;
...
impl pallet_polls::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type Currency = Balances;
	type Fungibles = Assets;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		Blogchain: blogchain,
		PalletProfile: pallet_profile,
		PalletTeamsAdvisors: pallet_teams_advisors,
		Ranks: pallet_ranks,
